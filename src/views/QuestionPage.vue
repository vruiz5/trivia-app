...
onMounted(async () => {
  ...
  answer.value = shuffle(answers.value)
...
<template>
  <div v-if="question" class="flex h-full w-full flex-col items-center gap-8 p-10">
    ...
    <div v-html="question.question" class="text-cent text-2xl font-bold"></div>
    <div class="grid w-full flex-grow grid-cols-2 gap-8">
        <div v-for="answer in answers" 
          v-html="answer.answer" 
          :key="answer.id" 
          class="bg-green-500 flex items-ceter justify-center text-4xl rounded-lg text-white py-10 px-2">
        </div>
    </div>
  </div>
  <div v-else class="">
